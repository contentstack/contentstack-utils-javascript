...
import { EmbeddedAsset, EmbedTagModel } from '../src/Models/embeddec-object-model';
...
describe('Embedded object render from content', () => {
    ...
    it('Find EmbedObject from undefinded data test', done => {
        ...
        renderModels = findEmbeddedObjects(assetDownloadJson as unknown as EmbedTagModel, undefined)
        expect(renderModels).toEqual([])
    ...
    it('Find EmbedObject from embed asset test', done => {
        let renderModels = findEmbeddedObjects(assetRichTextJson as unknown as EmbedTagModel, entryEmbeddedAssets)
        expect(renderModels).toEqual([entryEmbeddedAssets._embedded_assets[0]])
        renderModels = findEmbeddedObjects(entryRichTextJson as unknown as EmbedTagModel, entryEmbeddedEntries)
    ...
    it('Find Render string from default renderOption with alternate text', done => {
        const renderString = findRenderString(assetDisplayJson as unknown as EmbedTagModel, entryEmbeddedAssets._embedded_assets[0])
